...
def test_priority_score_is_six_and_gap_scores_are_rejected():
    assert _validate_response({"author_influence_score": 6}) == 6
    ...
    assert _validate_response({"author_influence_score": 8}) is None
...
def test_structured_output_schema_exposes_only_allowed_scores():
    ...
    assert score_schema["enum"] == [0, 1, 2, 3, 4, 6]
