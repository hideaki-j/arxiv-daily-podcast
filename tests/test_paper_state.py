...
def test_load_state_migrates_legacy_priority_scores_and_aggregate_totals(tmp_path):
    path = tmp_path / "state.json"
    path.write_text(
        """
        {
          ...
          "pooled_papers": {
            ...
            "2406.2": {
              ...
              "influence_score": 8,
              "scoring_scores": {"author_influence_score": 8, "legal_domain": 2},
              "scoring_total_score": 10.0,
              "ranking_scores": {"author_influence_score": 8, "legal_domain": 2},
              "ranking_total_score": 10.0
            }
          }
        }
        """
    ...
    state = load_paper_state(path)
    for base_id in ("2406.1", "2406.2"):
        record = state["pooled_papers"][base_id]
        assert record["influence_score"] == 6
        assert record["scoring_scores"]["author_influence_score"] == 6
        assert record["scoring_total_score"] == 8.0
        assert record["ranking_scores"]["author_influence_score"] == 6
        assert record["ranking_total_score"] == 8.0
...
def test_merge_pools_all_papers_and_stores_affiliations():
    ...
    assert state["pooled_papers"]["2406.12345"]["influence_score"] == 6
